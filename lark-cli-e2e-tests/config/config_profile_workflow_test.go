...
func TestConfig_ProfileWorkflow(t *testing.T) {
	...
	t.Run("rename and remove profile", func(t *testing.T) {
		...
		require.NoError(t, err)
		showRes.AssertExitCode(t, 2)
		assert.Contains(t, strings.ToLower(showRes.Stdout+showRes.Stderr), "no apps")

		finalListRes, err := clie2e.RunCmd(ctx, clie2e.Request{
			Args: []string{"profile", "list"},
		})
		require.NoError(t, err)
		finalListRes.AssertExitCode(t, 2)
		assert.Contains(t, strings.ToLower(finalListRes.Stdout+finalListRes.Stderr), "no apps")
	})
}
