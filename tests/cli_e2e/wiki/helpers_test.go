...
func createNode(t *testing.T, parentT *testing.T, ctx context.Context, spaceID string, data map[string]any) (gjson.Result, *clie2e.Result, error) {
	t.Helper()
	...
	parentT.Cleanup(func() {
		...
		deleteResult, deleteErr := deleteNodeAndVerify(cleanupCtx, spaceID, nodeToken, objType)
...
// createIsolatedParentUnderAnyHost creates an isolated parent under an existing
...
// test runs.
func createIsolatedParentUnderAnyHost(t *testing.T, parentT *testing.T, ctx context.Context, title string) (gjson.Result, gjson.Result) {
	t.Helper()

	hosts := listRootHosts(t, ctx)
	if len(hosts) == 0 {
		hosts = append(hosts, createReusableRootHost(t, ctx))
	}
	...
	for _, host := range hosts {
		...
		node, result, err := createNode(t, parentT, ctx, spaceID, map[string]any{
			"node_type":         "origin",
			"obj_type":          "docx",
			"title":             title,
		...
		if isSingleLayerLimitResult(result) {
...
func createReusableRootHost(t *testing.T, ctx context.Context) gjson.Result {
	t.Helper()
...
func listRootHosts(t *testing.T, ctx context.Context) []gjson.Result {
	t.Helper()
...
func isSingleLayerLimitResult(result *clie2e.Result) bool {
	if result == nil {
		return false
	}
...
func getNode(t *testing.T, ctx context.Context, nodeToken string) gjson.Result {
	t.Helper()
...
func getSpace(t *testing.T, ctx context.Context, spaceID string) gjson.Result {
	t.Helper()
...
func listSpaces(t *testing.T, ctx context.Context, pageSize int) gjson.Result {
	t.Helper()
...
// deleteNodeAndVerify removes a wiki node, then polls get_node until the
...
// children first.
func deleteNodeAndVerify(ctx context.Context, spaceID, nodeToken, objType string) (*clie2e.Result, error) {
	...
	if getResult.ExitCode != 0 || !isAPIEnvelopeSuccessful(getResult.Stdout) {
		if isNodeNotFoundResult(getResult) {
	...
	children, childListResult, childListErr := listNodeChildren(ctx, spaceID, originalNodeToken)
	if childListErr != nil || childListResult == nil || childListResult.ExitCode != 0 {
		return childListResult, childListErr
	}
	for _, child := range children {
		childDeleteResult, childDeleteErr := deleteNodeAndVerify(ctx, spaceID, child.NodeToken, child.ObjType)
		if childDeleteErr != nil || childDeleteResult == nil || (childDeleteResult.ExitCode != 0 && !isNodeNotFoundResult(childDeleteResult)) {
	...
	if deleteResult.ExitCode != 0 || !isAPIEnvelopeSuccessful(deleteResult.Stdout) {
		deleted, verifyErr := isNodeDeleted(ctx, originalNodeToken)
		if verifyErr != nil {
	...
	if err := waitNodeDeleted(ctx, originalNodeToken); err != nil {
		return deleteResult, err
	}
	return deleteResult, nil
}

func listNodeChildren(ctx context.Context, spaceID, parentNodeToken string) ([]nodeRef, *clie2e.Result, error) {
	...
	for {
		...
		if !isAPIEnvelopeSuccessful(result.Stdout) {
		...
		for _, item := range parsed.Get("data.items").Array() {
			...
			children = append(children, nodeRef{NodeToken: nodeToken, ObjType: objType})
		}
...
func waitNodeDeleted(ctx context.Context, nodeToken string) error {
	...
	for {
		deleted, err := isNodeDeleted(ctx, nodeToken)
		if err != nil {
			return err
		}
		if deleted {
...
func isNodeDeleted(ctx context.Context, nodeToken string) (bool, error) {
	result, err := clie2e.RunCmdWithRetry(ctx, clie2e.Request{
		Args:      []string{"api", "get", "/open-apis/wiki/v2/spaces/get_node"},
		DefaultAs: "bot",
		Params:    map[string]any{"token": nodeToken},
	...
	if result.ExitCode == 0 && isAPIEnvelopeSuccessful(result.Stdout) {
		return false, nil
	}
	if isNodeNotFoundResult(result) {
		return true, nil
	}
...
func isAPIEnvelopeSuccessful(stdout string) bool {
...
func isNodeNotFoundResult(result *clie2e.Result) bool {
	if result == nil {
		return false
	}
...
func findNodeByToken(t *testing.T, ctx context.Context, spaceID string, nodeToken string, parentNodeTokens ...string) gjson.Result {
	t.Helper()
