...
func TestSheets_SpreadsheetsResource(t *testing.T) {
	...
	t.Run("create spreadsheet with spreadsheets create as bot", func(t *testing.T) {
		folderToken := drive.CreateFolder(t, parentT, ctx, "lark-cli-e2e-sheets-resource-folder-"+suffix, defaultAs, "")
		result, err := clie2e.RunCmd(ctx, clie2e.Request{
		...
		parentT.Cleanup(func() {
			...
			deleteResult, deleteErr := drive.DeleteResourceAndVerify(cleanupCtx, spreadsheetToken, "sheet", defaultAs)
