...
// CreateFolder creates a Drive folder, optionally under a parent folder, and
// deletes it during parent cleanup.
func CreateFolder(t *testing.T, parentT *testing.T, ctx context.Context, name string, defaultAs string, parentFolderToken string) string {
	t.Helper()
	...
	parentT.Cleanup(func() {
		...
		deleteResult, deleteErr := DeleteResourceAndVerify(cleanupCtx, folderToken, "folder", defaultAs)
...
// DeleteResourceAndVerify deletes a Drive-backed resource, then polls
...
// exists.
func DeleteResourceAndVerify(ctx context.Context, token, docType, defaultAs string) (*clie2e.Result, error) {
	if defaultAs == "" {
		defaultAs = "bot"
	}
	...
	if deleteResult.ExitCode != 0 {
		deleted, verifyErr := isResourceDeleted(ctx, token, docType, defaultAs)
		if verifyErr != nil {
	...
	if err := waitResourceDeleted(ctx, token, docType, defaultAs); err != nil {
		return deleteResult, err
	}
	return deleteResult, nil
}

func waitResourceDeleted(ctx context.Context, token, docType, defaultAs string) error {
	...
	for {
		deleted, err := isResourceDeleted(ctx, token, docType, defaultAs)
		if err != nil {
			return err
		}
		if deleted {
...
func isResourceDeleted(ctx context.Context, token, docType, defaultAs string) (bool, error) {
	if defaultAs == "" {
		defaultAs = "bot"
	}
	result, err := clie2e.RunCmd(ctx, clie2e.Request{
